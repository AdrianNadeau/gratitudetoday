...
router.post('/updateAccount/', async function(req,res){
  ...
  if (!req.session.userid) {
      ...
      res.send(JSON.stringify(error.message));
        ...
        User.findOneAndUpdate({_id: mongoose.Types.ObjectId(req.body.id)}, {$set: {displayName: displayName, bio: req.body.bio, location: req.body.location, url:req.body.url}}, {new: true}, (err, doc) => {
        if (err) {
            logger.error(err.message);
        }
...
router.get('/deletePost/:postid',  async function(req, res) {
  ...
  try{
    ...
    await Post.deleteOne({_id: mongoose.Types.ObjectId(req.params.postid)}, function (err, post) {
