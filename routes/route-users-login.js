...
router.post('/',  [
  // Validate fields
  check('email').isEmail()
      .withMessage("Email must be valid and entered"),
...
[
  ...
  check('confirmpassword').isLength({ min: 6 })
        .withMessage("Confirm Password must be entered and at least 6 characters"),
    ...
    var pwd = req.body.password;
      ...
      if(pwd!=confirmPwd){
        ...
        bcrypt.hash(pwd, salt, function (err, hash) {
          User.findOneAndUpdate({_id: mongoose.Types.ObjectId(req.body._id)}, {$set: {password: hash}}, {new: true}, (err, doc) => {
